...
for (let i = 0; i < mouseArray.length; i++) {
    for (letj = 0; i < keyBoardArray.length; i++) {

        let sumComb = mouseArray[i] + keyBoardArray[j];
        if (sumComb > myBudjet)
            continue;
        else if (sumComb = myBudjet) {
            console.log(`mouseArray: ${mouseArray[i]}  keyBoardArray : ${keyBoardArray[i]} `)
            breake;
        }
        else {
            max = sumComb;
